...
@section('content')
   ...
   @php
      ...
      $allowed_roles = ['vendor', 'administrator'];
   ...
   <div class="container single-product">
         ...
         <div class="row justify-content-center m-0">
            <div class="publish">
               <div class="user-not-login">
                  ...
                  <a class="mt-2 login btn btn-blue" href="#" data-toggle="modal"
                     data-target="#LoginUser">{{ _e('Log In', 'premast') }}</a>
               </div>
            </div>
         </div>
      @elseif (array_intersect($allowed_roles, $user->roles))
         @php
            ...
            if ('POST' == $_SERVER['REQUEST_METHOD'] && !empty($_POST['action']) && $_POST['action'] == 'update_post' && $post_id != 'false') {
                ...
                $product = wp_update_post([
                    ...
                    'tax_input' => ['product_cat' => [$cat, $child, $children], 'product_tag' => $tags],
         ...
         <form id="publish_product" name="update_post" method="post" action=""
                     ...
                     <div class="col-12">
                        <label for="frontend-button" class="label-upload arrows right">
                           ...
                           <img class="profile-pic"
                                src="{{ Utilities::global_thumbnails($post->ID, 'full') }}">
                           <span>{{ _e('upload your cover image here', 'premast') }}</span>
                           <span>{{ _e('1104 × 944 pixels', 'premast') }}</span>
                        </label>
                        <div class="upload-form">
                           <div class="form-group">
                              <input type="file" id="frontend-button"
                                     class="files-thumbnail form-control" />
                              <input name="thumbnail" value="{{ get_post_thumbnail_id($post->ID) }}"
                                     id="thumbnails" hidden required />
                           </div>
                        </div>
                        <div class="upload-gallery mt-5 mb-0">
                           ...
                           <div class="input-group mb-3 mt-3">
                              <label class="frontend-gallery" for="frontend-gallery">
                                 <img class="profile-gallery"
                                      src="{{ get_theme_file_uri() . '/dist/images/upload-gallery.png' }}">
                              </label>
                              <div id="thumb-output">
                                 @php
                                 ...
                                 @foreach ($ids_gallery as $id_gallery)
                                    {!! wp_get_attachment_image($id_gallery, ['150', '150'], '', ['class' => 'thumb']) !!}
                                 @endforeach
                              </div>
                              <span class="gallery-files d-block"></span>
                              <div class="custom-file">
                                 <input type="text" id="frontend-gallery" hidden>
                                 <input name="galler" value="{{ $gallery_images }}" id="gallers"
                                        hidden />
                              </div>
                           </div>

                           {{-- //SECTION - slide gallery form --}}
                           @php
                              acf_form([
                                  'post_id' => $post->ID,
                                  'uploader' => 'wp',
                                  'fields' => ['field_63636bd7bc6e3'],
                                  'form' => false,
                              ]);
                           @endphp
                        ...
                        <div class="input-group mb-3">
                           @if ($youtube)
                              <input type="text" value="{{ $youtube['0'] }}" class="form-control"
                                     name="slide_gallery"
                                     placeholder="Embed Youtube or Slideshare URL">
                           @else
               ...
               <div class="summary entry-summary col-md-4 col-12 sidebar-shop mt-2">
                  <div class="download-product price-input arrows left">
                     <div class="input-group mb-3">
                        ...
                        <input type="number" value="{{ $price }}" name="prices"
                     ...
                     <textarea class="form-control" name="short_description" placeholder="Short description"
                               rows="3" required>{{ $post->post_excerpt }}</textarea>
                  </div>
                  <div class="input-group mb-4 mt-4">
                     <label class="custom-download-label arrows left mb-0" for="upload_file">
                        <div class="upload-response"></div>
                        <img class="profile-download"
                             src="{{ get_theme_file_uri() . '/dist/images/upload.png' }}">
                        <span class="name-files">{{ $download['name'] }}</span>
                        <span>{{ _e('Max size 1GB') }}</span>
                     </label>
                     <div class="custom-file d-none">
                        <input type="file" id="upload_file"
                               class="custom-file-input files-download" />
                        <input name="file_url" value="{{ $download['file'] }}" id="files_url" hidden
                               required />
                        <input name="file_name" value="{{ $download['name'] }}" id="files_name" hidden
                               required />
                     </div>
                  </div>
                  <div class="ads-block mb-3">
                     <div class="alert alert-light m-0 pt-2 pb-2 pl-0" role="alert">
                        {{ _e('Add Ads Items', 'premast') }}</div>
                     <label for="ads-button" class="label-ads">
                        <span class="images-files"></span>
                        <img class="ads-pic"
                             src="@if ($ads_image) {{ wp_get_attachment_image_url($ads_image, 'full') }} @else {{ get_theme_file_uri() . '/dist/images/upload-gallery.png' }} @endif">
                     </label>
                     <div class="upload-form">
                        <div class="form-group">
                           <input type="file" id="ads-button" class="files-ads form-control" />
                           <input name="ads_image"
                                  value="@if ($ads_image) {{ $ads_image }} @endif"
                                  id="ads" hidden />
                        </div>
                     </div>
                     <div class="input-group">
                        <input type="text" name="ads_link"
                               value="@if ($ads_link) {{ $ads_link }} @endif"
                  ...
                  ?>
                        <option value="<?= $child->term_id ?>" selected="selected">
                           <?= $child->name ?></option>
                        <?php endforeach; ?>
                     </select>
                     <?php else: ?>
                  ...
                  <div class="box-information mt-5">
                     ...
                     <div class="input-group mb-3 slide-info">
                        <input value="{{ $slide_type }}" type="text" name="slide_type"
                               class="form-control" placeholder="Compatible with">
                     </div>
                     <div class="input-group mb-3 slide-info">
                        <input value="{{ $slide_format }}" type="text" name="slide_format"
                               class="form-control" placeholder="Enter Format of Slides">
                     </div>
                     <div class="input-group mb-3">
                        <input value="{{ $slide_colors }}" type="text" class="form-control"
                               name="slide_colors" placeholder="Number of Slides">
                     </div>
                     <div class="input-group mb-3">
                        <select name="slide_number" id="">
                           <option value="{{ $slide_number }}" selected="selected">Animation
                           </option>
                  ...
                  <p class="mt-t col-12">
                     <input type="submit" value="Update" tabindex="6" id="submit"
                            name="submit" />
                  </p>

                  <p class="error-field">
                     <span id="error-headline" class="alert alert-danger"
                           style="display:none;">{{ _e('Alert headline field input required (kindly check!)', 'premast') }}</span>
                     <span id="error-thumb" class="alert alert-danger"
                           style="display:none;">{{ _e('Alert image field input required (kindly check!)', 'premast') }}</span>
                     <span id="error-description" class="alert alert-danger"
                           style="display:none;">{{ _e('Alert description field input required (kindly check!)', 'premast') }}</span>
                     <span id="error-file" class="alert alert-danger"
                           style="display:none;">{{ _e('Alert file field input required (kindly check!)', 'premast') }}</span>
                     <span id="error-category" class="alert alert-danger"
                           style="display:none;">{{ _e('Alert category field input required (kindly check!)', 'premast') }}</span>
                     <span id="error-tags" class="alert alert-danger"
                           style="display:none;">{{ _e('Alert tags field input required (kindly check!)', 'premast') }}</span>
                  </p>
...
<style>
   ...
   .acf-gallery-attachment.-image.ui-sortable-handle {
      flex: 1 1 20%;
      min-width: 200px;
   }
