...
@endphp
<pmst-header></pmst-header>


<script>
   let test_users_ids = [1, 2226, 3625, 3210, 2804, 2269, 217, 1010];
   ...
   header.addEventListener('google-signin', function(e) {
      ...
      googleLink.href =
         `https://premast.com/wp/wp-login.php?loginSocial=google&redirect=${currentUrl}`;
   ...
   header.addEventListener('search', function(e) {
      let search = e.detail
      let search_link = `https://premast.com/templates/search/?refine=${search}`
      if (test_users_ids.includes(userId)) {
         search_link = `https://premast.com/templates/search?refine=${search}`
      }
