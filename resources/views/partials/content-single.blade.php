                ...
                <div class="col-md-12 col-12 row m-0">
                    <div class="head col-md-7 col-12 p-0">
                        <p class="label label-term mb-0">
                            @if ($categories)
                                ...
                                @endforeach
                            @endif
                            @if (get_field('time_read_article'))
                                <time class="text-light">|
                                    {{ the_field('time_read_article') }}</time>
                            @endif
                        </p>
                    ...
                    <div class="user-info col-12 p-0">
                        @if ($user_name)
                            ...
                            {{ _e('By:', 'premast') }} {{ $user_name }}
                        @endif
                    </div>
                    <div class="date-info col-12 p-0">
                        <time>{{ the_time('d M, Y') }} </time>
                        @if (get_field('time_read_blog'))
                            <time> - {{ the_field('time_read_blog') }}</time>
                        @endif
                    </div>
                </div>
                ...
                <div class="col-md-8 col-12">
                    ...
                    @if (!empty($thumb_url))
                        <img src="{{ $thumb_url }}" alt="{{ the_title() }}"
                            class="img-fluid img-post wp-post-thumbnail">
                    @endif
                ...
                <div class="col-md-4 col-12">
                    ...
                    @if ($related_post->have_posts())
                        <div class="related-single">
                            <h3 class="headline-related col-12">
                                {{ _e('related Posts', 'premast') }}</h3>
                            <ul class="list-unstyled">
                                @while ($related_post->have_posts())
                                    @php $related_post->the_post() @endphp
                                    <li class="media">
                                        <img src="{{ Utilities::global_thumbnails(get_the_ID(), 'thumbnail') }}"
                                            class="mr-3"
                                            alt="{!! get_the_title() !!}<">
